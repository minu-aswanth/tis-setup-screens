      ...
      //  console.log(onstr,temponstr);
        if(onstr !== temponstr && ctr == 0){
           ...
           // console.log(temponstr+'asdasd')
            setAlerts(false,false,false,true,false);
            asound.play();
        }
    },300000);
...
$(document).ready(function(){
    $.ajax({
        url: '../utils/nms_atcc_devices.php',
        success: function(result) {
            if(result.length == 2){
...
$(document).ready(function(){
    ...
    create_report = function(){
        ...
        if(formatted_fromDate > formatted_toDate){
            ...
            $.ajax({
                url: '../utils/createreport_nms_atcc.php',
                            ...
                            $.ajax({
                                ...
                                success: function(result) {
                                    if(result.length == 2){
                                        ...
                                        $.ajax({
                                            url: '../utils/createreport_nms_atcc.php',
                                            data :{fromDate:formatted_startDate,toDate:formatted_endDate,device:device,status:'all',parameter:"1",orderby:'',forgraph:'0'},
                                            type: 'POST',
                                            success: function(result_count){
                                                if(result_count.length == 2){
                                                    alert("Connection Error please try again");
				...
				$.ajax({
					...
					success: function(result) {
						if(result.length == 2){
							...
							json_to_csv(result, "Fault Report - ATCC", true,fromDate,toDate);
						}
					}
            	});
			}
                ...
                $.ajax({
                  url: '../utils/get_username.php',
                    success: function(result_username) {
                        ...
                        window.open("../utils/createreport_nms_atcc_pdf.php?fromDate=" + formatted_fromDate +"&toDate=" + formatted_toDate +"&username="+username+"&status="+status+"&device="+device+'&orderby='+radioVal,'_blank');    
                    }
                });
            }
        }
...
function json_to_csv(json_data, title, label, fromDate, toDate) {
  ...
  //  if(document.getElementById("des").checked == true){
    ...
    var file_name = "fault_report_atcc_"+fromDate+"_"+toDate;  
        ...
        if (timeArray_next.hasOwnProperty(k)) {
            if(timeArray_next[k]=="0"){
                ...
                $.ajax({
                    ...
                    data :{module:"detector",device:deviceSCN,startDate:formatted_startDate},
                    type: 'POST',
