$(document).ready(function(){
	fetch_signalraw = function(){
				$('#nms_signal').empty();
                $.ajax({
                	url: '../utils/nms_signal.php',
...
$(document).ready(function(){
    $.ajax({
        url: '../utils/nms_signal_devices.php',
        success: function(result) {
            if(result.length == 2){
...
$(document).ready(function(){
    pagenation = function(page_number){
        ...
        var header = '<tr><td bgcolor="#993232"><font color="white">SCN</td><td bgcolor="#993232"><font color="white">Status</td><td bgcolor="#993232"><font color="white">Last_Updated</td></tr>';
        ...
        for(i=(page_number-1)*10; i<=initial_value;i++){
            ...
            append += '<td>'+faultText+'</td>'
            append += '<td>'+data_queried[data_queried.length-i-1].Last_Updated+'</td>'
            append += '</tr>'
    ...
    create_report = function(){
        ...
        if(formatted_fromDate > formatted_toDate){
            ...
            $.ajax({
                url: '../utils/createreport_nms_signal.php',
                data :{fromDate:formatted_fromDate,toDate:formatted_toDate,device:device,status:status, parameter:"0"},
                type: 'POST',
                success: function(result) {
                    if(result.length == 2){
                        alert("Connection Error please try again");
                            ...
                            $.ajax({
                                ...
                                success: function(result) {
                                    if(result.length == 2){
                                        ...
                                        $.ajax({
                                            url: '../utils/createreport_nms_signal.php',
                                            data :{fromDate:formatted_startDate,toDate:formatted_endDate,device:device,status:'all',parameter:"1"},
                                            type: 'POST',
				...
				$.ajax({
					...
					success: function(result) {
						if(result.length == 2){
							...
							json_to_csv(result, "Fault Report - signal", true,fromDate,toDate);
						}
					}
            	});
			}
                ...
                $.ajax({
                  url: '../utils/get_username.php',
                    success: function(result_username) {
                        ...
                        window.open("../utils/createreport_nms_signal_pdf.php?fromDate=" + formatted_fromDate +"&toDate=" + formatted_toDate +"&username="+username+"&status="+status+"&device="+device,'_blank');    
                    }
                });
            }
        }
...
function json_to_csv(json_data, title, label, fromDate, toDate) {
    ...
    var file_name = "fault_report_signal_"+fromDate+"_"+toDate;  
        ...
        if (timeArray_next.hasOwnProperty(k)) {
            if(timeArray_next[k]=="0"){
                ...
                $.ajax({
                    ...
                    data :{module:"signal",device:deviceSCN,startDate:formatted_startDate},
                    type: 'POST',
