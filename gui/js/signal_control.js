$( document ).ready(function() {
	...
	$.ajax({
		...
		success: function(result) {
			...
			for (var i = 0; i < signals.length; i++) {
				$('.group_table tbody').append('<tr><td colspan="1"><input type="radio" name="groups"></td><td colspan="2" group_id='+signals[i].GroupID+'>'+(i+1)+'</td><td colspan="4">'+signals[i].GroupName+'</td><td colspan="4">'+signals[i].NumPlans+'</td></tr>')
			}
		}
	});
	$('.add_group').click(function(){
		...
		$.ajax({
			...
			success: function(result) {
				if(result != 0){
					alert("Group Name Already Exists try again.");
					$('.group_name').val("")
					$('.group_name').focus();
					return;
				}
	...
	insert_group = function(group_name, num_plans, post_times, message){
		var string = '{'
		...
		$.ajax({
			...
			data :{group_name:group_name,num_plans:num_plans,string:string},
			type: 'POST',
			success: function(result) {
				if(result.includes("success")){
					alert(message);
