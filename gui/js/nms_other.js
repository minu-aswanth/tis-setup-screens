$(document).ready(function(){
	...
	fetch_otherraw = function(){
				$('#nms_other').empty();
                $.ajax({
                	url: '../utils/nms_other.php',
                	success: function(result) {
						...
						for(i=0; i<raw_set.length;i++){
							...
							append = '<tr>'
                            append += '<td>'+raw_set[i].IPAddress+'</td>'
      ...
      //  console.log(onstr,temponstr);
        if(onstr !== temponstr && ctr == 0){
           ...
           // console.log(temponstr+'asdasd')
            setAlerts(false,false,true,false,false);
            asound.play();
        }
    },300000);
...
$(document).ready(function(){
    $.ajax({
        url: '../utils/nms_other_devices.php',
        success: function(result) {
            if(result.length == 2){
                ...
                for(i=0;i<scns.length;i++){
                    $("#scns").append('<option>'+scns[i].IPAddress+'</option>');
                }
            }
        }
    });
...
$(document).ready(function(){
    pagenation = function(page_number){
        $('#searchResult').empty();
        //$('#searchResult').append('<table border="0"><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr></table>');
        var header = '<tr><td bgcolor="#993232"><font color="white">IP Address</td><td bgcolor="#993232"><font color="white">Status</td><td bgcolor="#993232"><font color="white">Status Start Time</td><td bgcolor="#993232"><font color="white">Status End Time</td><td bgcolor="#993232"><font color="white">Total Time(minutes)</td></tr>';
        $('#searchResult').append(header);
        // if(document.getElementById("des").checked == true){
        //     var faultText = null;
        //     var num = data_queried.length-1;
        //     var num_pages = Math.ceil(data_queried.length/10);

        //     initial_value = Math.min((num_pages-page_number)*10, data_queried.length-1)
        //     for(i=initial_value+4; i>=(initial_value)-5  && i>=0;i--){
        //         if(data_queried[i].online === "0"){
        //             faultText = '<img src="../images/offline.png"/>';
        //         }else{
        //             faultText = '<img src="../images/online.png"/>';
        //         }
        //         append = '<tr>'
        //         append += '<td>'+data_queried[i].IPAddress+'</td>'
        //         append += '<td>'+faultText+'</td>'
        //         append += '<td>'+data_queried[i].Last_Updated+'</td>'
        //         append += '</tr>'
        ...
        for(i=(page_number-1)*10; i<=initial_value;i++){
            ...
            append = '<tr>'
            append += '<td>'+data_queried[data_queried.length-i-1].IPAddress+'</td>'
            append += '<td>'+faultText+'</td>'
    ...
    create_report = function(){
        ...
        if(formatted_fromDate > formatted_toDate){
            ...
            $.ajax({
                url: '../utils/createreport_nms_other.php',
                ...
                success: function(result) {
                    if(result.length == 2){
                        ...
                        if(devices_set.length > 0){
                            ...
                            var endDate = toDate;
                            var scn = device;
                            var formatted_startDate = formatted_fromDate + " 00:00:00";
                            ...
                            $.ajax({
                                ...
                                data :{formatted_startDate:formatted_startDate,formatted_endDate:formatted_endDate,deviceType:deviceType,scn:scn,orderby:radioVal},
                                type: 'POST',
                                success: function(result) {
                                    if(result.length == 2){
                                        alert("Connection Error please try again");
                                        ...
                                        for(i=devices_set.length-1; i>=0;i--){
                                            onlineKey = devices_set[i].IPAddress+'_1';
                                            offlineKey = devices_set[i].IPAddress+'_0';
                                            timeArray[onlineKey] = 0;
                                            timeArray[offlineKey] = 0;
                                        }
                                        $.ajax({
                                            url: '../utils/createreport_nms_other.php',
                                            data :{fromDate:formatted_startDate,toDate:formatted_endDate,device:device,status:'all',parameter:"1",orderby:radioVal,forgraph:'1'},
                                            type: 'POST',
                                                    ...
                                                    for(var i=time_set.length-1;i>=0;i--){
                                                        if(i == 0){
                                                            if(time_set[i].online=="0"){
                                                                onlineKey = time_set[i].IPAddress+'_1';
                                                                ...
                                                                offlineKey = time_set[i].IPAddress+'_0';
                                                            ...
                                                            if(time_set[i].IPAddress != time_set[i-1].IPAddress){
                                                                if(time_set[i].online == "0"){
                                                                    onlineKey = time_set[i].IPAddress+'_1';
                                                                    var time = timeArray[onlineKey];
                                                                    timeArray[onlineKey]=Math.round(time) + Math.round(new Date(time_set[i].Last_Updated).getTime()/60000) - Math.round(new Date(formatted_startDate).getTime()/60000);
				...
				$.ajax({
					...
					success: function(result) {
						if(result.length == 2){
							...
							json_to_csv(result, "Fault Report - other", true,fromDate,toDate);
						}
					}
            	});
			}
                ...
                $.ajax({
                  url: '../utils/get_username.php',
                    success: function(result_username) {
                        ...
                        window.open("../utils/createreport_nms_other_pdf.php?fromDate=" + formatted_fromDate +"&toDate=" + formatted_toDate +"&username="+username+"&status="+status+"&device="+device+"&orderby="+radioVal,'_blank');    
                    }
                });
            }
        }
...
function json_to_csv(json_data, title, label, fromDate, toDate) {
  ...
  //  if(document.getElementById("des").checked == true){
    ...
    var file_name = "fault_report_other_"+fromDate+"_"+toDate;  
        ...
        if (timeArray_next.hasOwnProperty(k)) {
            if(timeArray_next[k]=="0"){
                ...
                $.ajax({
                    ...
                    data :{module:"other",device:deviceSCN,startDate:formatted_startDate},
                    type: 'POST',
                        ...
                        if(result_lastStatus.length == 2){
                            ...
                            for(i=lastStatus.length-1; i>=0;i--){
                                if(lastStatus[i].online == "1"){
                                    onlineKey = lastStatus[i].IPAddress+'_1';
                                    ...
                                    offlineKey = lastStatus[i].IPAddress+'_0';
    ...
    $.ajax({
        ...
        success: function(result_devices){
            ...
            for(var i=raw_set.length-1;i>=0;i--){
                ...
                if(timeArray_next[raw_set[i].IPAddress+"_0"] == 0 && timeArray_next[raw_set[i].IPAddress+"_1"] == 0){
                    offlinePercent = 0;
                    onlinePercent = 0;
                } else {
                    offlinePercent = ((timeArray_next[raw_set[i].IPAddress+"_0"])*100)/(timeArray_next[raw_set[i].IPAddress+"_0"] + timeArray_next[raw_set[i].IPAddress+"_1"]);
                
                    onlinePercent = ((timeArray_next[raw_set[i].IPAddress+"_1"])*100)/(timeArray_next[raw_set[i].IPAddress+"_0"] + timeArray_next[raw_set[i].IPAddress+"_1"]);
                }
                ...
                devices.push(raw_set[i].IPAddress);

                totalOnline = totalOnline + timeArray_next[raw_set[i].IPAddress+"_1"];
                totalOffline = totalOffline + timeArray_next[raw_set[i].IPAddress+"_0"];
