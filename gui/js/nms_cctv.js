      ...
      //  console.log(onstr,temponstr);
        if(onstr !== temponstr && ctr == 0){
           ...
           // console.log(temponstr+'asdasd')
            setAlerts(false,true,false,false,false);
            asound.play();
        }
    },10000);
...
$(document).ready(function(){
    $.ajax({
        url: '../utils/nms_cctv_devices.php',
        success: function(result) {
            if(result.length == 2){
...
$(document).ready(function(){
    pagenation = function(page_number){
        $('#searchResult').empty();
        //$('#searchResult').append('<table border="0"><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr><tr><td colspan="3">&nbsp;<td></tr></table>');
    ...
    create_report = function(){
        ...
        if(formatted_fromDate > formatted_toDate){
            ...
            $.ajax({
                url: '../utils/createreport_nms_cctv.php',
				...
				$.ajax({
					...
					success: function(result) {
						if(result.length == 2){
							...
							json_to_csv(result, "Fault Report - CCTV", true,fromDate,toDate);
						}
					}
            	});
			}
                ...
                $.ajax({
                  url: '../utils/get_username.php',
                    success: function(result_username) {
                        ...
                        window.open("../utils/createreport_nms_cctv_pdf.php?fromDate=" + formatted_fromDate +"&toDate=" + formatted_toDate +"&username="+username+"&status="+status+"&device="+device+'&orderby='+radioVal,'_blank');    
                    }
                });
            }
        }
...
function json_to_csv(json_data, title, label, fromDate, toDate) {
   ...
   // if(document.getElementById("des").checked == true){
    ...
    var file_name = "fault_report_cctv_"+fromDate+"_"+toDate;  
        ...
        if (timeArray_next.hasOwnProperty(k)) {
            if(timeArray_next[k]=="0"){
                ...
                $.ajax({
                    ...
                    data :{module:"cctv",device:deviceSCN,startDate:formatted_startDate},
                    type: 'POST',
