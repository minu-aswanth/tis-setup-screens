      ...
      //  console.log(onstr,temponstr);
        if(onstr !== temponstr && ctr == 0){
           ...
           // console.log(temponstr+'asdasd')
            setAlerts(false,false,false,false,true);
            asound.play();
        }
    },300000);
...
$(document).ready(function(){
    ...
    create_report = function(){
        ...
        if(formatted_fromDate > formatted_toDate){
            ...
            $.ajax({
                url: '../utils/createreport_nms_met.php',
                            ...
                            $.ajax({
                                ...
                                success: function(result) {
                                    if(result.length == 2){
                                        ...
                                        $.ajax({
                                            url: '../utils/createreport_nms_met.php',
                                            data :{fromDate:formatted_startDate,toDate:formatted_endDate,device:device,status:'all',parameter:"0",orderby:'',forgraph:'1'},
                                            type: 'POST',
				...
				$.ajax({
					...
					success: function(result) {
						if(result.length == 2){
							...
							json_to_csv(result, "Fault Report - MET", true,fromDate,toDate);
						}
					}
            	});
			}
                ...
                $.ajax({
                  url: '../utils/get_username.php',
                    success: function(result_username) {
                        ...
                        window.open("../utils/createreport_nms_met_pdf.php?fromDate=" + formatted_fromDate +"&toDate=" + formatted_toDate +"&username="+username+"&status="+status+"&device="+device+'&orderby='+radioVal,'_blank');    
                    }
                });
            }
        }
...
function json_to_csv(json_data, title, label, fromDate, toDate) {
    ...
    var file_name = "fault_report_met_"+fromDate+"_"+toDate;  
        ...
        if (timeArray_next.hasOwnProperty(k)) {
            if(timeArray_next[k]=="0"){
                ...
                $.ajax({
                    ...
                    data :{module:"meteorological",device:deviceSCN,startDate:formatted_startDate},
                    type: 'POST',
