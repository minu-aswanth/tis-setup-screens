...
function add_user(){
    try {
        ...
        while($row=$handle->fetch()){
            ...
            if((strcmp(trim($module),"ECB") === 0) or (strcmp(trim($module),"Admin") === 0) or (strcmp(trim($module),"Maintenance") === 0)){

                $handle2=$link->prepare("SELECT A.`sr_number`,E.SystemCodeNumber,A.`module`,A.`subject`,A.`created_date`,A.`status`,A.`user_created`,E.online,E.LastUpdated FROM `tis_maintenance_sr` A , tis_ecb_fault E JOIN (select SystemCodeNumber, MAX(LastUpdated) as LastUpdated from tis_ecb_fault GROUP BY SystemCodeNumber) t2 ON E.LastUpdated = t2.LastUpdated and E.SystemCodeNumber=t2.SystemCodeNumber WHERE A.module='ECB' and A.status='Open' and A.`device`=E.SystemCodeNumber order by A.sr_number Desc");
