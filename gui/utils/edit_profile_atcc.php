...
try {
	...
	$link = linkToTIS();
    $handle=$link->prepare("UPDATE `utmc_detector_profile_data` SET `ShortDescription`='$shortDescription',`LongDescription`='$longDescription',`SystemCodeNumber`='$scn',`TotalFlowLb`='$RoadTempLb',`TotalFlowPr`='$RoadTempPr',`TotalFlowUb`='$RoadTempUb',`SpeedLb`='$PrecipitationLb',`SpeedPr`='$PrecipitationPr',`SpeedUb`='$PrecipitationUb',`FromTimeSun`='$sun_from',`ToTimeSun`='$sun_to',`FromTimeMon`='$mon_from',`ToTimeMon`='$mon_to',`FromTimeTue`='$tue_from',`ToTimeTue`='$tue_to',`FromTimeWed`='$wed_from',`ToTimeWed`='$wed_to',`FromTimeThu`='$thu_from',`ToTimeThu`='$thu_from',`FromTimeFri`='$fri_from',`ToTimeFri`='$fri_to',`FromTimeSat`='$sat_from',`ToTimeSat`='$sat_to' WHERE `ProfileId`='$profileID'"); 
    $handle->execute();
	return "success";
}
